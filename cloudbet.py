...
def get_odds_cloudbet():
    ...
    event_url = f"https://sports-api.cloudbet.com/pub/v2/odds/events?sport=volleyball&live=false&limit=1000&from={int(time.time())}&to={int(time.time()+3600*24)}"
                ...
                odds.append(
                    {
                        ...
                        "odds": [{
                            "gameID": "tbd",
                            "value": selection.get("outcome").replace("_", "/").replace("=", ": ").capitalize() + " | " if len(selection.get("params").replace("=", ": ")) > 3 else "" + selection.get("params").replace("=", ": "),
